...
public class PlayConfigure extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		{
			levels=combolevel.getSelectedItem().toString();
			if (def.isSelected()) 
			{
				GuestShooting g = new GuestShooting(name, "10x10", 4, Integer.parseInt(comboShootCount.getSelectedItem().toString()),levels);
			} 
			else 
			if (f10.isSelected()) 
			{
				GuestShooting g = new GuestShooting(name,"10x10", Integer.parseInt(co10.getSelectedItem().toString()), Integer
							.parseInt(comboShootCount.getSelectedItem().toString()),levels);
			} 
			else 
			if (f15.isSelected()) 
			{
				GuestShooting g = new GuestShooting(name, "15x15", Integer.parseInt(co15.getSelectedItem().toString()),
							Integer.parseInt(comboShootCount.getSelectedItem().toString()),levels);
			} 
			frameo.setVisible(false);
		}
		else 
		if (e.getActionCommand() == "Statistic view") 
		{
